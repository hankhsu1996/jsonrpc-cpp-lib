...
// Helper function to create a temporary socket path
auto GetTempSocketPath() -> std::string {
  static int counter = 0;
  return "/tmp/jsonrpc_test_socket_" + std::to_string(counter++);
}

TEST_CASE(
    "FramedPipeTransport can send and receive a single message",
    "[FramedPipeTransport]") {
  std::string socket_path = GetTempSocketPath();
  spdlog::info(
      "Test: FramedPipeTransport single message - using socket path: {}",
      socket_path);

  // Create server in a separate thread
  std::thread server_thread([&socket_path]() {
    try {
      spdlog::info(
          "Server: Creating transport on socket path: {}", socket_path);
      auto server_transport =
          std::make_unique<jsonrpc::transport::FramedPipeTransport>(
              socket_path, true);
  ...
  try {
    spdlog::info(
        "Client: Creating transport to connect to socket path: {}",
        socket_path);
...
TEST_CASE(
    "FramedPipeTransport can handle bidirectional communication",
    "[FramedPipeTransport]") {
  std::string socket_path = GetTempSocketPath();
  spdlog::info(
      "Test: FramedPipeTransport bidirectional - using socket path: {}",
      socket_path);
...
TEST_CASE(
    "FramedPipeTransport server can send multiple messages to client",
    "[FramedPipeTransport]") {
  std::string socket_path = GetTempSocketPath();
  spdlog::info(
      "Test: FramedPipeTransport server multiple messages - using socket path: "
      "{}",
      socket_path);
  ...
  // Create server in a separate thread
  std::thread server_thread([socket_path, &server_messages]() {
    try {
      spdlog::info(
          "Server: Creating transport on socket path: {}", socket_path);
      auto server_transport =
  ...
  std::thread client_thread([socket_path, &server_messages, &received_messages,
                             &test_completed]() {
    try {
      spdlog::info(
          "Client: Creating transport to connect to socket path: {}",
